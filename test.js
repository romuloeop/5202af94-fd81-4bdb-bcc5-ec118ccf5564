...
function runTest() {
    ...
    apiClient.get(batchUrl, {params: {ids: ["fileid1","fileid2"]}}).then(response => console.log(response.data));
    
    // Should return [{id:"fileid2"}]
    // apiClient.get(batchUrl, {params: {ids: ["fileid2"]}}).then(response => console.log(response.data));
    // Should reject as the fileid3 is missing from the response
    // apiClient.get(batchUrl, {params: {ids: ["fileid3"]}}).then(response => console.log(response.data));
}
